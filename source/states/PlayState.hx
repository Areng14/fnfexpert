...
class PlayState extends MusicBeatState
{
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		var healper:String = "50";
		if (totalPlayed != 0) {
			healper = Std.string(Math.round(healthBar.percent));
	...
	function noteMiss(daNote:Note, isDanger:Bool = false):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		//Dupe note remove
		notes.forEachAlive(function(note:Note) {
		...
		noteMissCommon(daNote.noteData, daNote, isDanger);
	...
	function noteMissCommon(direction:Int, note:Note = null, isDanger:Bool = false)
	{
		// Initialize subtract variable with default value
		var subtract:Float = 0.05;

		// if difficulty is higher than hard make note miss cost more
		if (PlayState.storyDifficulty >= 3) {
			subtract = 0.25;
		}

		if (isDanger == true) {
			//50 health gone from danger >:)
			subtract = 0.5;
		}

		// no miss note logic

		//checks if note is no miss AND immune notes are on
		if (note.noteType == "No Miss" && ClientPrefs.data.immunityNotes == true) {
			combo = 0;
			...
			//yeah uhh it does shit but doesnt damage player

			//if instakill on miss is on kill the player since its still a miss
			if(instakillOnMiss)
				{
					vocals.volume = 0;
					opponentVocals.volume = 0;
			...
			//if not no miss
			PlayState.instance.health -= subtract;
		}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(note.hitCausesMiss) {
			...
			if (ClientPrefs.data.deathOnDanger) {
				health = 0;
				vocals.volume = 0;
				opponentVocals.volume = 0;
				doDeathCheck(true);
